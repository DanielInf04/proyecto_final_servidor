...
class CheckoutController extends Controller
{
    public function store(Request $request)
    {
        ...
        foreach ($request->productos as $producto) {
            ...
            if (!$productoBD) {
                return response()->json([
                    'error' => 'El producto con ID' . $producto['producto_id'] . ' no existe.'
                ], 404);
            }

            if ($productoBD->stock < $producto['cantidad']) {
                return response()->json([
                    'error' => 'No hay suficiente stock para el producto: ' . $productoBD->nombre,
        ...
        return DB::transaction(function () use ($request) {
            ...
            // SI se ha enviado un cupón, comprobamos si ya fue usado

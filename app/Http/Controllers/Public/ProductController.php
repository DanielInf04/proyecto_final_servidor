...
class ProductController extends Controller
{
    ...
    public function recommended(Request $request)
    {
        ...
        $recomendados = $productos->map(function ($producto) use ($rutaImagenes) {
            $primeraImagen = $producto->imagenes->first();
            $imagenUrl = $primeraImagen ? asset($rutaImagenes . '/' . $primeraImagen->imagen) : null;
            ...
            return [
                'id' => $producto->id,
                'nombre' => $producto->nombre,
                'precio_con_iva' => round($producto->precio_base * (1 + $iva), 2),
                'imagen' => $imagenUrl,
                'valoracion_promedio' => $producto->resenyas->avg('valoracion')
            ];
        });
